...
export type AssignmentPolicy = (subjectName: Name, userId: string) => Promise<Name | undefined>
...
export class ServerOidcChallenge implements ServerChallenge<State> {
  ...
  public async process(
    ...
    const { 'oidc-id': oidcId, 'access-code': accessCode, 'redirect-uri': redirectUri } = request.parameters
    console.info(
    ...
    try {
      ...
      if (data['error']) {
        ...
        try {
          const assignedName = await this.options.assignmentPolicy?.(context.subjectName, String(payload['email']))
          if (assignedName) {
            ...
            console.info(`Rename the certificate to ${assignedName.toString()}`)
          }
        } catch {
